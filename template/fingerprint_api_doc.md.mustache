...
------------- | ------------- | -------------
**opts** | **...ConfigOption** | Optional functional options that modify the configuration. Defaults to none.
...
Set the base URL for the API Client. Useful for testing purposes. Do not use in production.
...
Set a custom HTTP client for making requests.

`func WithHTTPClient(httpClient *http.Client) ConfigOption`

WithHTTPClient sets a custom HTTP client for making requests. The client's transport will be wrapped to add SDK identification.

### WithFingerprintAPI

Set a custom Fingerprint API implementation.
...
Enable or disable debug mode for the API Client.
...
AsErrorResponse attempts to convert an error to an ErrorResponse and returns a boolean indicating success.
...
`AsErrorResponse` can be used for all methods, not only `GetEvent`. To learn more, refer to example located in [example/getEventWithError.go](../example/getEventWithError.go).
...
> Event GetEvent(ctx, eventID, opts)
...
To learn more, refer to example located in [example/getEvent.go](../example/getEvent.go).
...
**opts** | **...GetEventOption** | Optional functional options that modify the query parameters. Defaults to none.
...
Provide a `rulesetID` query parameter to evaluate the event against a ruleset. To learn more, refer to example located in [example/getEventWithRulesetEvaluation.go](../example/getEventWithRulesetEvaluation.go).

`func WithRulesetID(rulesetID string) GetEventOption`

WithRulesetID sets the ruleset used to evaluate the event
...
> EventSearch SearchEvents(ctx, req)

Search events using a `SearchEventRequest`.

> SearchEventRequest NewSearchEventsRequest()

Create a new search request, use builder pattern to set optional parameters.
...
func main() {
	...
	sDKPlatform := SearchEventsSdkPlatform("js") // SearchEventsSDKPlatform | Filter events by the SDK Platform associated with the identification event (`sdk.platform` property) . `js` - Javascript agent (Web). `ios` - Apple iOS based devices. `android` - Android based devices.  (optional)
	...
	searchReq := fingerprint.NewSearchEventsRequest().
        ...
        TotalHits(totalHits).
        TorNode(torNode)
	resp, r, err := client.SearchEvents(context.Background(), req)
	if err != nil {
...
To learn more, refer to example located in [example/searchEvent.go](../example/searchEvent.go).
...
------------- | ------------- | -------------
**ctx** | **context.Context** | context for authentication, logging, cancellation, deadlines, tracing, etc.
**req** | **SearchEventRequest** | The search request containing all query parameters. Build using `NewSearchEventsRequest()` and optional builder methods.
...
> UpdateEvent(ctx, eventID, eventUpdateReq)
...
To learn more, refer to example located in [example/updateEvent.go](../example/updateEvent.go).
...
**eventUpdateReq** | **[EventUpdate](EventUpdate.md)** | The event update request.
...
To learn more, refer to example located in [example/deleteVisitorData.go](../example/deleteVisitorData.go).
