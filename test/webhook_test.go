...
func TestIsValidWebhookSignature(t *testing.T) {
	...
	t.Run("With valid signature", func(t *testing.T) {
		isEqual := fingerprint.IsValidWebhookSignature(validHeader, []byte(data), secret)

		assert.Equal(t, true, isEqual)
	})

	t.Run("With invalid header", func(t *testing.T) {
		isEqual := fingerprint.IsValidWebhookSignature("v2=invalid", []byte(data), secret)

		assert.Equal(t, false, isEqual)
	})

	t.Run("With header without version", func(t *testing.T) {
		isEqual := fingerprint.IsValidWebhookSignature("invalid", []byte(data), secret)

		assert.Equal(t, false, isEqual)
	})

	t.Run("With empty header", func(t *testing.T) {
		isEqual := fingerprint.IsValidWebhookSignature("", []byte(data), secret)

		assert.Equal(t, false, isEqual)
	})

	t.Run("With empty secret", func(t *testing.T) {
		isEqual := fingerprint.IsValidWebhookSignature(validHeader, []byte(data), "")

		assert.Equal(t, false, isEqual)
	})

	t.Run("With empty data", func(t *testing.T) {
		isEqual := fingerprint.IsValidWebhookSignature(validHeader, []byte(""), secret)
