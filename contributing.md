...
- [example](./example) - folder contains examples for common use cases like `GetEvent` as well as uncommon use cases like `GetEventWithRulesetEvaluation`. Each example is its own subdirectory with a `main.go` (e.g. `example/getEvent/main.go`), so every example is an independent `package main`.
...
- This repo is a Go workspace (`go.work`). Any new modules also need a step added to [lint.yml](./.github/workflows/lint.yml). The module list is hardcoded to keep lint as one check with inline annotations instead of one per module.
